...
async function bootstrap(): Promise<void> {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .setDescription(
      'Topcoder forums API. The service exposes topic read routes protected by `read:forums-topics` plus transactional topic, post, watch, and read-state mutations with centralized forums authorization, runtime member/IP ban checks, locked-topic mutation restrictions, inherited topic restrictions, challenge/resource access checks, granular M2M scopes, and post-commit best-effort watch notification emails for created posts and child-topic starter posts. Dedicated moderation management routes let human administrators or M2M callers with `moderate:forums` lock/unlock topics and manage member or exact-IP bans; challenge copilots do not gain moderation-endpoint access. Topic detail embeds posts under `read:forums-topics`; `read:forums-posts` remains reserved for future post-specific reads. Health and readiness remain DB-only.',
    )
    .setVersion('6.0')
    .addBearerAuth({
      type: 'http',
